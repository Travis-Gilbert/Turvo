...
//! Turvo: a Tauri [`Runtime`] backed by an in-process Servo engine.
//!
//! The low-level runtime implementation is derived from
//! `copse-dev/tauri-runtime-servo` at commit
//! `b9d4ef11a5493b730172dca2dcba927987b93d1f`. Turvo keeps the original
//! dual-license notices and layers a stable application entry point over that
//! proven Servo/Tao integration.
//!
//! Turvo is experimental. In particular, runtime API coverage and behavior are
//! tracked separately from cross-platform compile coverage.
...
///
/// ```ignore
/// tauri::Builder::<turvo::Turvo>::new()
///   .invoke_system(turvo::INVOKE_SYSTEM_SCRIPT)
///   .run(tauri::generate_context!())
///   .unwrap();
/// ```
pub const INVOKE_SYSTEM_SCRIPT: &str = include_str!("./invoke-system-initialization-script.js");

/// The exact Servo crate release selected by this Turvo release.
///
/// A public constant lets diagnostics and applications report the renderer
/// without reaching into Cargo metadata. A test keeps this value aligned with
/// the exact dependency requirement in `Cargo.toml`.
pub const SERVO_VERSION: &str = "0.5.0";

/// The concrete Servo-backed runtime used by [`builder`].
pub type Turvo = Servo<tauri::EventLoopMessage>;

/// Creates a Tauri builder configured to use Turvo's Servo runtime and IPC
/// bridge.
///
/// Existing Tauri applications still disable Tauri's default `wry` feature in
/// `Cargo.toml`, then replace `tauri::Builder::new()` with this function.
#[must_use]
pub fn builder() -> tauri::Builder<Turvo> {
  tauri::Builder::<Turvo>::new().invoke_system(INVOKE_SYSTEM_SCRIPT)
}

/// Creates a Tauri builder after configuring Turvo's process-wide engine
/// options.
///
/// Configuration is rejected after the first Servo webview has started so
/// every window in an application uses the same engine settings.
pub fn builder_with_options(
  options: TurvoOptions,
) -> std::result::Result<tauri::Builder<Turvo>, OptionsAlreadyLocked> {
  devtools::configure(options)?;
  Ok(builder())
}
pub use url;
