...
impl ServoDelegate for EngineDelegate {
  ...
  fn notify_devtools_server_started(&self, port: u16, token: String) {
    log::info!("Servo devtools server listening on 127.0.0.1:{port}");
    self
      .options
      .notify_devtools_server_started(self.webview_id.clone(), port, token);
  }

  fn request_devtools_connection(&self, request: AllowOrDenyRequest) {
    if self.options.approve_devtools_connection() {
      request.allow();
    } else {
      request.deny();
    }
...
impl Embedder {
  ...
  fn build(
    ...
    // retry look like a second engine or lock configuration permanently.
    let runtime_options = crate::devtools::options_for_engine();
    if let Some(address) = runtime_options.devtools_listen_address() {
...
mod tests {
  ...
  fn ipc_request_preserves_a_remote_page_origin() {
    ...
    assert_eq!(request.uri().path(), "/app/index.html");
  }
